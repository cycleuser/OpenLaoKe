...
- 双模式运行 - 在线模式（直接API）和本地模式（原子分解）
- HyperAuto 模式 - 完全自主运行，自我改进
- 反AI检测 - 确保生成内容不被检测为AI生成
- 任务监督 - 自动重试和完成验证（仅本地模式）
- 模型能力评估 - 根据模型能力调整任务粒度（仅本地模式）
...
- 直接API调用，无中间处理
- 不进行意图解析、任务分解
- 适合强大的云端模型（GPT-4o、Claude 4、MiniMax-M2.7等）
- 快速响应，单次API调用
...
- 多模型协作（规划器 + 执行器）
- 任务监督和自动重试
- 适合小型本地模型（Gemma 1B、Llama 3.2 1B等）

**关键代码位置**：
- `openlaoke/core/state.py` - `local_mode: bool` 字段
- `openlaoke/entrypoints/cli.py` - `--local` 参数
- `openlaoke/core/repl.py` - 模式判断逻辑
...
### 模型切换无效
**问题**：`/model` 切换模型后API调用仍使用旧模型

**原因**：只更新了 `session_config.model`，未更新 `multi_provider_config.active_model`，且 `send_message` 未传递 model 参数

**解决方案**：
1. `/model` 命令同时更新 `session_config.model` 和 `multi_provider_config.active_model`
2. `repl.py` 调用 `send_message` 时传递 `model=self.app_state.session_config.model`

### 非英文输入触发错误流程
**问题**：中文等非英文输入被错误地识别为 `WRITE_PROGRAM` 意图，触发 atomic 命令

**原因**：翻译后文本包含 "write" 关键词，被 intent parser 错误匹配

**解决方案**：
1. 非英文输入不触发 atomic 命令
2. 同时保留原文和译文发送给模型

### 在线模式被本地模式功能干扰
**问题**：意图解析、任务分解等影响在线模式体验

**解决方案**：添加 `--local` 模式标志，在线模式完全跳过本地模式特性
