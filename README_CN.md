...
这个项目最早的想法可以追溯到 2024 年，真正开始写代码要到 2026 年 4 月。提交记录讲的是四个阶段的故事：**长大 → 专精 → 收敛 → 简化**。

**第一阶段——造引擎（2024 年酝酿，2026 年 4 月落地）。** 首次提交落地了一个与提供商无关的智能体循环。它很快就长出了模型侧的机械：CPU/GPU 混合推理、智能模型选择、批量操作、双模型协作、模型预热、原子生成，以及 HyperAuto（一个自主的自我改进模式）。
