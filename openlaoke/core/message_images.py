...
_IMAGE_PATTERN = re.compile(r"!?\[.*?\]\(([^\s)]+\.(?:png|jpg|jpeg|gif|webp|bmp|svg))\)", re.IGNORECASE)
_FILE_REF_PATTERN = re.compile(r"(?:attach|image|see|view|open)\s+([^\s]+\.(?:png|jpg|jpeg|gif|webp|bmp|svg))", re.IGNORECASE)
...
def encode_image_base64(file_path: str) -> str | None:
    ...
    try:
        ...
        mime_map = {"png": "image/png", "jpg": "image/jpeg", "jpeg": "image/jpeg", "gif": "image/gif", "webp": "image/webp", "bmp": "image/bmp", "svg": "image/svg+xml"}
...
def model_supports_vision(model: str) -> bool:
    ...
    vision_models = {
        ...
        "claude-3", "claude-3.5", "claude-3-opus", "claude-sonnet", "claude-haiku",
        "gemini", "gemma3", "llava", "bakllava", "cogvlm", "qwen-vl", "pixtral",
        "llama3.2-vision",
    }
